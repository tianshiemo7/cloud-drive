...
const { safePath, isPreviewable, md5File, upload, handleMulterError } = require('./utils');
...
async function processUploadedFiles(files, req, res) {
  ...
  for (const file of files) {
    const allKeys = Object.values(keys).map(m => typeof m === 'string' ? m : m.key);
    let fileKey;
    do { fileKey = crypto.randomBytes(3).toString('hex'); } while (allKeys.includes(fileKey));

    const md5 = await md5File(file.path);

    keys[file.filename] = {
      key: fileKey,
      folder: config.clusterNodeId || 'local',
...
function rekeyHandler(req, res) {
  ...
  let newKey;
  do { newKey = crypto.randomBytes(3).toString('hex'); } while (allKeys.includes(newKey));

  const expiresAt = expiresIn ? new Date(Date.now() + expiresIn * 1000).toISOString() : null;

  keys[filename] = {
    key: newKey,
    createdAt: (typeof keys[filename] === 'object' && keys[filename].createdAt) || new Date().toISOString(),
    expiresAt,
    oneTime: !!oneTime,
    downloadCount: 0,
    maxDownloads: maxDownloads || null,
    md5: (typeof keys[filename] === 'object' && keys[filename].md5) || null
  };
