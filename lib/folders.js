...
function deleteFolderHandler(req, res) {
  ...
  // 将该文件夹下的文件移回节点根目录
  const rootId = config.clusterNodeId || 'local';
  const keys = loadKeys();
  ...
  for (const [filename, meta] of Object.entries(keys)) {
    ...
    if (m.folder === folderId) {
      if (typeof keys[filename] === 'string') {
        keys[filename] = { key: keys[filename], folder: rootId };
      } else {
        keys[filename].folder = rootId;
      }
      keysChanged = true;
    }
  }
...
function moveFileHandler(req, res) {
  ...
  const targetFolder = isRoot ? (config.clusterNodeId || 'local') : folder;
